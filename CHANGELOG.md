...
- **Configurable listen threshold** — `playback.history_min_listen_seconds` (default 5) sets how long a track must play before it counts as a play instead of a skip, for both local history and the YT Music account sync. Clamped to 0–3600; `0` still requires at least one full second of playback. Thanks @Villoh (#116).
